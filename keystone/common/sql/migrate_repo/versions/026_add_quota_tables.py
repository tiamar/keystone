...
def upgrade(migrate_engine):
    ...
    resources_table = sql.Table(
        ...
        sql.Column('type', sql.Enum('ABSOLUTE', 'COUNTABLE', 'RESERVABLE'),
                   nullable=False),
    ...
    quotas_table = sql.Table(
        ...
        meta,
        sql.Column('id', sql.String(64), primary_key=True),
        sql.Column('resource_id', sql.String(64),
                   sql.ForeignKey('resources.id'),
                   nullable=False),
    ...
    child_field_data_table = sql.Table(
        ...
        meta,
        sql.Column('id', sql.String(64), primary_key=True),
        sql.Column('quota_id', sql.String(64), sql.ForeignKey('quotas.id'),
                   nullable=False),
